...
func (r *router) matchAndDispatch(messages <-chan withContextData[*packets.PublishPacket], order bool, client *client) <-chan withContextData[*PacketAndToken] {
	var wg sync.WaitGroup
	ackOutChan := make(chan withContextData[*PacketAndToken]) // Channel returned to caller; closed when messages channel closed
	var ackInChan chan withContextData[*PacketAndToken]       // ACKs generated by ackFunc get put onto this channel
	...
	if order {
		...
		ackInChan = make(chan withContextData[*PacketAndToken])
	...
	go func() { // Main go routine handling inbound messages
		...
		for message := range messages {
			...
			sent := false
			r.RLock()
			m := messageFromPublish(message.v, ackFunc(message.ctx, ackInChan, client.persist, message.v))
			for e := r.routes.Front(); e != nil; e = e.Next() {
				if e.Value.(*route).match(message.v.TopicName) {
					if order {
			...
			if !sent {
				if r.defaultHandler != nil {
					if order {
						...
						wg.Add(1)
						go func() {
							r.defaultHandler(message.ctx, client, m)
							if !client.options.AutoAckDisabled {
								m.Ack()
							}
							wg.Done()
